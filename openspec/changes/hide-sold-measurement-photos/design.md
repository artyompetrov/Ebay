...
**6. Narrow `MeasurementPhotoService`'s dependency to a single-method port instead of the full `IMeasurementQueries`.**
`IMeasurementQueries` has 9 methods; `MeasurementPhotoService` only ever calls `GetMeasurementInfo`. Depending on the full interface forced the hand-written test fake in `MeasurementPhotoServiceTests` (this project has no mocking library) to stub 8 unrelated methods with `NotSupportedException` just to satisfy the type - an ISP violation surfaced by writing a real test double. Extracted `IMeasurementInfoQueries` (one method: `GetMeasurementInfo`) in `Server.Application.Abstractions.Driven.Abstractions.Queries`; `IMeasurementQueries : IMeasurementInfoQueries` inherits it, so the existing EF adapter (`MeasurementQueries`) needs no change and is registered against both interfaces in DI. `MeasurementPhotoService` now depends on `IMeasurementInfoQueries` only, and its test fake shrinks to the one method it actually uses. `MeasurementPlotService` keeps depending on the full `IMeasurementQueries` (it genuinely uses 4 of its methods), so it is intentionally left alone.

**7. Cap cache lifetime with the same policy already used for the state-dependent chart endpoints.**
Both photo endpoints' responses are state-dependent (real bytes vs. placeholder), but originally shipped with no explicit `Cache-Control` directive at all. A browser or intermediary cache could then keep serving pre-sale bytes for an unbounded time after the measurement becomes `Sold`, since the URL never changes and nothing tells the cache to revalidate - silently defeating the whole point of this change. `MeasurementPageController.GetEbayCurves`/`GetEbayTubeDescription` already face the identical problem for the charts and already solve it the same bounded way: `[ResponseCache(Duration = 60 * 5)]` (5 minutes), compiled in for Release builds only (`#if !DEBUG`). Apply the exact same attribute, duration, and `#if !DEBUG` guard to both photo endpoints, so a sale is reflected to any cache within the same 5-minute window the project already accepts for charts. Alternative considered: `Cache-Control: no-store`/must-revalidate for stronger consistency. Rejected in favor of matching the existing, already-accepted chart policy instead of introducing a second, inconsistent caching strategy for what is the same kind of state-dependent public image.
