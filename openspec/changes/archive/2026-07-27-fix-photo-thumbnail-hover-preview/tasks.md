...
- [~] 3.4 Run `./agent-check.sh` from the repo root before opening the PR. (Skipped per user instruction. The one earlier clean run — before the checkbox/backdrop/instruction-text refinements — passed: build clean, 29 unit + 12 integration tests passed, Chrome extension built. Two reruns after the refinements failed for environmental reasons unrelated to the code: an IOException from the CLI's console pipe, then a file lock held by a leftover local dev-server process from manual testing. `dotnet build` was independently re-verified clean after every subsequent edit.)
