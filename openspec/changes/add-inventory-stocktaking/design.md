...
### 4. Write-model migration

`InventoryDate` is added as a plain nullable `timestamptz` column on the measurement table in `Server.Adapters.Driven.EF.WriteModel` (schema `wm`), via a standard EF Core migration (`dotnet ef migrations add`). No backfill is needed — existing rows get `NULL` (never inventoried), which is a valid, meaningful state ("not yet counted").
