...
Register a *separate* `IMemoryCache` (via a keyed singleton, `MemoryCacheExtensions.ServiceKey`) with `MemoryCacheOptions.SizeLimit` set, and use only this instance from `MeasurementPhotoService` (new) and `MeasurementPlotService` (as an L1 in front of the existing `DbCache`, which remains as the restart-durable L2).
