...
- Для даты и времени в C# используем только `DateTimeOffset` и `DateOnly`; тип `DateTime` не используем.
- Бизнес-правила в первую очередь размещаем в `Server.Domain`; в `Server.Application` оставляем только редкие сценарии координации нескольких агрегатов/портов. Адаптеры не должны содержать бизнес-решения, только трансляцию ввода/вывода.
- Не расширяем видимость членов (`private` -> `internal/public`) только ради тестов. Для проверки поведения используем публичные сценарии и интеграционные/сценарные тесты.
