...
## Создание лота для продажи из UI
- Лот для продажи создается на вкладке `Measurements` конкретного продукта.
- Перед этим нужно создать сам продукт и открыть его страницу замеров формата `/Product/{productId}/Measurements`.
- В блоке `Лоты для продажи` пользователь вводит только название лота.
- Поле `Id` пользователь не вводит: 7-символьный идентификатор генерируется автоматически в агрегате `LotForSale` при создании и дальше не изменяется.
