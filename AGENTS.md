...
Дополнительно стараемся придерживаться правила: **один класс/интерфейс/record на файл** (особенно для новых контрактов и адаптеров), чтобы упрощать навигацию и ревью.
...
6. Полезные замечания для отладки:
   ...
   - Для интеграционных тестов, где нужно дождаться eventual consistency, ретраи делаем через `Tests.Integration/TestHelpers.RetryUntilValidationSuccessAsync`, а не через самописные циклы ожидания.
